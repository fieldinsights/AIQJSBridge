...
- (NSURL*)urlForRecording:(NSString*)resourcePath
{
    ...
    NSString* docsPath = [NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES)[0] stringByAppendingPathComponent:folder];
    ...
    if ([[resourcePath pathExtension] caseInsensitiveCompare:RECORDING_WAV] != NSOrderedSame) {
        ...
        if (![filePath hasPrefix:docsPath]) {
            NSLog(@"Resource for recording outside of the documents folder");
            return nil;
        }
        ...
        if (filePath == nil) {
            NSLog(@"Resource for recording outside of the filesystem root");
            return nil;
        }
    } else {
        // if resourcePath is not from FileSystem put in tmp dir, else attempt to use provided resource path
        NSString* tmpPath = [[NSTemporaryDirectory()stringByStandardizingPath] stringByAppendingPathComponent:folder];
        resourcePath = [resourcePath stringByStandardizingPath];
        if (([resourcePath hasPrefix:tmpPath]) || ([resourcePath hasPrefix:docsPath])) {
            filePath = resourcePath;
        } else {
            // put in temp dir
            resourcePath = [[NSString stringWithFormat:@"%@/%@", tmpPath, resourcePath] stringByStandardizingPath];
            if ([resourcePath hasPrefix:tmpPath]) {
                filePath = resourcePath;
            } else {
                NSLog(@"Resource for recording outside of the temporary folder");
            }
        }
    }
...
- (NSURL*)urlForPlaying:(NSString*)resourcePath
{
    ...
    if ([resourcePath hasPrefix:HTTP_SCHEME_PREFIX] || [resourcePath hasPrefix:HTTPS_SCHEME_PREFIX]) {
        ...
        if (filePath == nil) {
            ...
            NSString *tempPath = [NSTemporaryDirectory() stringByAppendingPathComponent:folder];
            NSString* testPath = [[tempPath stringByAppendingPathComponent:resourcePath] stringByStandardizingPath];
            if (! [testPath hasPrefix:tempPath]) {
                NSLog(@"Resource for recording outside of the temporary folder");
                return nil;
            }
