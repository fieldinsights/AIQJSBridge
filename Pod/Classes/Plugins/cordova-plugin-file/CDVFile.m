...
{
    NSString *folder = [[NSString stringWithFormat:@"%02lX", (long)[AIQSession currentSession].sessionId.hash] stringByAppendingPathComponent:@"cordova"];
//    NSString *libPath = [NSSearchPathForDirectoriesInDomains(NSLibraryDirectory, NSUserDomainMask, YES) objectAtIndex:0];
    NSString *docPath = [[NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES) objectAtIndex:0] stringByAppendingPathComponent:folder];
    return @{
//        @"library": libPath,
//        @"library-nosync": [libPath stringByAppendingPathComponent:@"NoCloud"],
        ...
        @"cache": [[NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES) objectAtIndex:0] stringByAppendingPathComponent:folder],
        @"bundle": [[NSBundle mainBundle] pathForResource:@"AIQJSBridge" ofType:@"bundle"],
//        @"root": @"/"
    };
}
...
- (void)pluginInitialize
{
    ...
    self.appLibraryPath = [[[paths objectAtIndex:0] stringByAppendingPathComponent:folder] stringByAppendingPathComponent:@"files"];

    // Get the Temporary directory path
    self.appTempPath = [[NSTemporaryDirectory()stringByAppendingPathComponent:folder]stringByStandardizingPath];   // remove trailing slash from NSTemporaryDirectory()

    // Get the Documents directory path
    paths = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES);
    self.rootDocsPath = [[paths objectAtIndex:0] stringByAppendingPathComponent:folder];
    ...
    if ([location isEqualToString:@"library"]) {
         ...
         *  versions.
         */
        if ([[NSFileManager defaultManager] createDirectoryAtPath:self.rootDocsPath
                                      withIntermediateDirectories:YES
                                                       attributes:nil
                                                            error:&error]) {
            [self registerFilesystem:[[CDVLocalFilesystem alloc] initWithName:@"persistent" root:self.rootDocsPath]];
        } else {
            NSLog(@"Unable to create documents directory: %@", error);
        }
    } else {
        NSAssert(false,
...
//    NSString* libPathNoSync = [libPath stringByAppendingPathComponent:@"NoCloud"];
    NSString* docPath = [NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES)[0] stringByAppendingPathComponent:folder];
//    NSString* storagePath = [libPath stringByDeletingLastPathComponent];
    NSString* cachePath = [NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES)[0] stringByAppendingPathComponent:folder];
...
//        @"syncedDataDirectory": [[NSURL fileURLWithPath:libPathSync] absoluteString],
        ...
        @"tempDirectory": [[NSURL fileURLWithPath:[NSTemporaryDirectory() stringByAppendingPathComponent:folder]] absoluteString]
    };
