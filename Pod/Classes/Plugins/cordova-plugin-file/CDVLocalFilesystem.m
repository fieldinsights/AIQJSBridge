...
- (id) initWithName:(NSString *)name root:(NSString *)fsRoot
{
    if (self) {
        _name = name;
        _fsRoot = [fsRoot stringByStandardizingPath];
    }
    return self;
}
...
- (NSDictionary *)makeEntryForLocalURL:(CDVFilesystemURL *)url {
    ...
    BOOL bExists = (path != nil) && [fileMgr fileExistsAtPath:path isDirectory:&isDir];
    if (bExists) {
...
- (CDVPluginResult *)getFileForURL:(CDVFilesystemURL *)baseURI requestedPath:(NSString *)requestedPath options:(NSDictionary *)options
{
    ...
    if ([requestedPath rangeOfString:@":"].location != NSNotFound) {
        ...
        CDVFilesystemURL* requestedURL = [self URLforFullPath:[self fullPathForFileSystemPath:combinedPath]];
        
        NSFileManager* fileMgr = [[NSFileManager alloc] init];
        BOOL bIsDir;
        BOOL bCorrectPath = (requestedURL != nil);
        BOOL bExists = bCorrectPath && [fileMgr fileExistsAtPath:[self filesystemPathForURL:requestedURL] isDirectory:&bIsDir];
        if (! bCorrectPath) {
            // path points outside of the filesystem root - return SECURITY_ERR
            errorCode = SECURITY_ERR;
        } else if (bExists && (create == NO) && (bIsDir == !bDirRequest)) {
...
- (CDVPluginResult*)getParentForURL:(CDVFilesystemURL *)localURI
{
    ...
    NSString *filesystemPath = [self filesystemPathForURL:newURI];
    if (! filesystemPath) {
        return [CDVPluginResult resultWithStatus:CDVCommandStatus_ERROR messageAsInt:SECURITY_ERR];
    }
...
{
    fsPath = [[self.fsRoot stringByAppendingPathComponent:fsPath] stringByStandardizingPath];
...
- (CDVPluginResult *)truncateFileAtURL:(CDVFilesystemURL *)localURI atPosition:(unsigned long long)pos
{
    ...
    unsigned long long newPos = [self truncateFile:fileSystemPath atPosition:pos];
...
- (void)copyFileToURL:(CDVFilesystemURL *)destURL withName:(NSString *)newName fromFileSystem:(NSObject<CDVFileSystem> *)srcFs atURL:(CDVFilesystemURL *)srcURL copy:(BOOL)bCopy callback:(void (^)(CDVPluginResult *))callback
{
    ...
    if (! destRootPath) {
        callback([CDVPluginResult resultWithStatus:CDVCommandStatus_ERROR messageAsInt:SECURITY_ERR]);
        return;
    }
