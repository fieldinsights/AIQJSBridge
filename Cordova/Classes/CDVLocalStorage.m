...
+ (NSMutableArray*)createBackupInfoWithCloudBackup:(BOOL)cloudBackup
{
    ...
    Class AIQSession = NSClassFromString(@"AIQSession");
    SEL selector = NSSelectorFromString(@"currentSession");
    NSMethodSignature *signature = [AIQSession methodSignatureForSelector:selector];
    NSInvocation *invocation = [NSInvocation invocationWithMethodSignature:signature];
    [invocation setSelector:selector];
    [invocation setTarget:AIQSession];
    [invocation invoke];
    __unsafe_unretained id session;
    [invocation getReturnValue:&session];
    NSString *sessionId = [session valueForKey:@"sessionId"];
    NSString *folder = [[NSString stringWithFormat:@"%02lX", (long)sessionId.hash] stringByAppendingPathComponent:@"cordova"];
    NSString* appLibraryFolder = [[NSSearchPathForDirectoriesInDomains(NSLibraryDirectory, NSUserDomainMask, YES) objectAtIndex:0] stringByAppendingPathComponent:folder];
    NSString* appDocumentsFolder = [[NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES) objectAtIndex:0] stringByAppendingPathComponent:folder];
    NSString* cacheFolder = [[appLibraryFolder stringByAppendingPathComponent:@"Caches"] stringByAppendingPathComponent:folder];
    NSString* backupsFolder = [[appDocumentsFolder stringByAppendingPathComponent:@"Backups"] stringByAppendingPathComponent:folder];
